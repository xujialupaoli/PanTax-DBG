#themis
cd ${workpath}/themis
/usr/bin/time -v -o query_time.log themis profile  -r ./reads/reads.fq  --single\
--db-prefix  ./themis_db \
...
/usr/bin/time -v -o query_time.log_ganonclassify ganon classify --db-prefix $db -s $read --output-prefix results --report-type abundance -t $threads 2>&1 |tee log_ganonclassify/usr/bin/time -v -o query_time.log_ganonreport ganon report -i results.rep --db-prefix $db --output-prefix tax_profile --report-type abundance -r all  2>&1 |tee log_ganonreport
...
/usr/bin/time -v -o query_time.log centrifuger -k 1 -x $db -u $read -t 64 > cls.tsv
...
/usr/bin/time -v -o query_time.log $tools profile $db $read  -o result.tsv
...
/usr/bin/time -v -o query_time.log ${tools} --db $Kraken2_DB --output kraken2_query_reads --report kraken2_query_report --threads $threads $read
python time_process.py query_time.log > time_evaluation.txt
