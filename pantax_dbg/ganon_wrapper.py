...
# def _check_ganon_available():
    ganon = _ganon_bin()
    if not Path(ganon).exists():
        raise SystemExit(
            f"[PanTax-DBG][error] cannot find bundled DBG-ganon: {ganon}. "
            "Please check your PanTax-DBG installation."
        )
    return ganon
...
def run_classify_paired(
    ...
    """
    pair: ganon classify -d DB -p R1 R2 --output-prefix OUT --report-type TYPE -t N
    """
    ganon = _check_ganon_available()
...
def run_report_and_postprocess(
    ...
    if not Path(rep_in).exists():
        raise SystemExit(f"[PanTax-DBG][error] cannot find REP: {rep_in}")
    ...
    if not Path(tre_file).exists():
        raise SystemExit(f"[PanTax-DBG][error] ganon report did not create: {tre_file}")
