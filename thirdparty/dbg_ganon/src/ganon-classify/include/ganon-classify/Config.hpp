...
public:
    ...
    uint16_t    multi_pick            = 3;      // for multi-match reads, pick top-N strains to count multi_support
    ...
    bool validate()
    {
        ...
        if ( species_topk < 1 ) species_topk = 1;
        if ( multi_pick < 1 ) multi_pick = 1;
...
inline std::ostream& operator<<( std::ostream& stream, const Config& config )
{
    ...
    stream << "--output-species-all  " << config.output_species_all << newl;
    stream << "--output-species-topk " << config.output_species_topk << newl;
    stream << "--species-topk        " << config.species_topk << newl;
    stream << "--multi-pick          " << config.multi_pick << newl;
