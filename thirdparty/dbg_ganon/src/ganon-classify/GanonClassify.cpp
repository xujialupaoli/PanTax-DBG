...
{
    uint32_t unique_support = 0;
    uint32_t multi_support  = 0;
};

using TStrain2Species = robin_hood::unordered_map<std::string, std::string>; // strain_taxid -> species_taxid
using TPerSpeciesSupport =
    robin_hood::unordered_map<std::string, robin_hood::unordered_map<std::string, StrainSupport>>;

// climb parents until rank == "species" (or stop at root)
static inline std::string to_species( std::string const& node,
                                      TTax const& tax,
                                      std::string const& tax_root_node )
{
    std::string cur = node;
    auto it = tax.find(cur);
    ...
    for ( size_t guard = 0; guard < 128; ++guard )
    {
        ...
        if ( it2 == tax.end() ) break;

        if ( it2->second.rank == "species" ) return cur;

        std::string const& p = it2->second.parent;
        if ( p.empty() || p == cur ) break;
        if ( p == tax_root_node ) break;
        cur = p;
    }
    return node; // fallback
}

// stable tie-break: hash(readID + "|" + strainID)
static inline uint64_t stable_hash( std::string const& readID, std::string const& strainID )
{
...
void print_hierarchy( Config const& config, auto const& parsed_hierarchy )
{
    ...
    for ( auto const& hierarchy_config : parsed_hierarchy )
    {
        ...
        if ( !config.output_prefix.empty() )
        {
            ...
            if ( config.output_species_all )
                std::cerr << ", " << (config.output_prefix + ".species.all");
            if ( config.output_species_topk )
                std::cerr << ", " << (config.output_prefix + ".species_top10_strains.tsv");
            std::cerr << newl;
        }
    }
    std::cerr << "----------------------------------------------------------------------" << newl;
...
void classify( std::vector< Filter< TFilter > >& filters,
               ...
               SafeQueue< ReadOut >&             classified_species_queue,
               ...
               bool                              hierarchy_last,
               TPerSpeciesSupport&               per_species_support,
               TStrain2Species const&            strain2species )
{
        ...
        for ( size_t readID = 0; readID < rb.ids.size(); ++readID )
        {
            ...
            if ( max_count_read > 0 )
            {
                ...
                if ( count_filtered_matches > 0 )
                {
                    ...
                    // (2) folded species-level .species.all  + strain topK support counting
                    if ( config.output_species_all || config.output_species_topk )
                    {
                        // group strains by species for this read
                        // species -> max_kmer_count
                        robin_hood::unordered_map<std::string, size_t> sp_max;
                        sp_max.reserve(8);

                        // also prepare sorted candidates for multi-pick counting
                        // (species, strain, kmer_count)
                        std::vector<std::tuple<std::string,std::string,size_t>> candidates;
                        candidates.reserve( read_out.matches.size() );

                        for ( auto const& m : read_out.matches )
                        {
                            auto itsp = strain2species.find( m.target );
                            // if mapping missing, skip (shouldn't happen if tax is ok)
                            if ( itsp == strain2species.end() )
                                continue;

                            std::string const& sp = itsp->second;

                            // update species max score
                            auto it = sp_max.find(sp);
                            if ( it == sp_max.end() || m.kmer_count > it->second )
                                sp_max[sp] = m.kmer_count;

                            candidates.emplace_back( sp, m.target, m.kmer_count );
                        }
                        ...
                        if ( config.output_species_all )
                        {
                            ReadOut sp_out( rb.ids[readID] );
                            sp_out.matches.clear();
                            sp_out.matches.reserve( sp_max.size() );
                            for ( auto const& [sp, sc] : sp_max )
                                sp_out.matches.push_back( ReadMatch{ sp, sc } );

                            classified_species_queue.push( std::move(sp_out) );
                        }
                        // support counting for topK strains per species
                        // unique read: count unique_support for that strain
                        if ( config.output_species_topk )
                        {
                            if ( read_out.matches.size() == 1 )
                            {
                                auto const& m = read_out.matches[0];
                                auto itsp = strain2species.find( m.target );
                                if ( itsp != strain2species.end() )
                                {
                                    per_species_support[itsp->second][m.target].unique_support += 1;
                                }
                            }
                            else if ( read_out.matches.size() > 1 )
                            {
                                // multi-pick: sort by kmer_count desc, then stable hash for tie
                                std::sort( candidates.begin(), candidates.end(),
                                           [&](auto const& a, auto const& b)
                                           {
                                               auto const& spA = std::get<0>(a);
                                               auto const& stA = std::get<1>(a);
                                               auto kcA = std::get<2>(a);

                                               auto const& spB = std::get<0>(b);
                                               auto const& stB = std::get<1>(b);
                                               auto kcB = std::get<2>(b);

                                               if ( kcA != kcB ) return kcA > kcB;
                                               // tie-break: hash
                                               return detail::stable_hash(rb.ids[readID], stA) < detail::stable_hash(rb.ids[readID], stB);
                                           });

                                // pick top-N overall strains (not per species), count into their species buckets
                                uint16_t picked = 0;
                                for ( auto const& [sp, st, kc] : candidates )
                                {
                                    (void)kc;
                                    per_species_support[sp][st].multi_support += 1;
                                    picked++;
                                    if ( picked >= config.multi_pick ) break;
                                }
                            }
                        }
                    }
...
bool ganon_classify( Config config )
{
    ...
    for ( auto& [hierarchy_label, hierarchy_config] : parsed_hierarchy )
    {
        ...
        if ( config.output_species_all || config.output_species_topk )
        {
            strain2species.reserve( tax.size() );

            // map every node to its species ancestor (works for strain nodes)
            for ( auto const& [node, nd] : tax )
            {
                (void)nd;
                std::string sp = detail::to_species( node, tax, config.tax_root_node );
                if ( sp != node ) // only keep those that resolve to a species (typical for strains)
                    strain2species[node] = sp;
            }
        }
        if ( !config.skip_lca )
        {
        ...
        // ------------------------------------------------------------------
        // Exchange queues instance pointers for each hierarchy (if not first)
        //  - pointer_current: where reads are coming from
        //  - pointer_helper : where leftover reads go to (next hierarchy)
        // ------------------------------------------------------------------
        if ( !hierarchy_first )
        {
        ...
        // ------------------------------------------------------------------
        // Output queues (must be declared BEFORE writer threads)
        // ------------------------------------------------------------------
        SafeQueue< detail::ReadOut > classified_all_queue;
        SafeQueue< detail::ReadOut > classified_lca_queue;
        SafeQueue< detail::ReadOut > classified_species_queue;

        // ------------------------------------------------------------------
        // Writer threads (keep ofstream alive until writer threads finish)
        // ------------------------------------------------------------------
        std::vector< std::future< void > > write_tasks;

        std::ofstream out_species_all;
        bool species_all_opened = false;
        ...
        if ( !config.output_prefix.empty() )
        {
            ...
            // Species-level folded output
            if ( config.output_species_all )
            {
                std::string sp_all_path = config.output_prefix + ".species.all";
                if ( parsed_hierarchy.size() > 1 && !config.output_single )
                    sp_all_path = config.output_prefix + "." + hierarchy_label + ".species.all";

                if ( hierarchy_first || !config.output_single )
                    out_species_all.open( sp_all_path );
                else
                    out_species_all.open( sp_all_path, std::ofstream::app );

                species_all_opened = true;

                write_tasks.emplace_back( std::async( std::launch::async,
                                                      detail::write_classified,
                                                      std::ref( classified_species_queue ),
                                                      std::ref( out_species_all ) ) );
            }
        }
        ...
        std::vector< detail::TPerSpeciesSupport > supports( config.threads );        
        ...
        for ( size_t taskNo = 0; taskNo < config.threads; ++taskNo )
        {
            ...
            tasks.emplace_back( std::async( std::launch::async,
                                            ...
                                            hierarchy_first,
                                            hierarchy_last,
                                            std::ref( supports[taskNo] ),
                                            std::ref( strain2species ) ) );
        }
        ...
        classified_species_queue.notify_push_over();
        ...
        // Merge per-thread supports
        detail::TPerSpeciesSupport merged_support;
        if ( config.output_species_topk )
        {
            for ( auto& s : supports )
            {
                for ( auto& [sp, m] : s )
                {
                    for ( auto& [st, sup] : m )
                    {
                        merged_support[sp][st].unique_support += sup.unique_support;
                        merged_support[sp][st].multi_support  += sup.multi_support;
                    }
                }
            }

            // Write topK file (once, on last hierarchy or just once if single hierarchy)
            // Here we write once per run (hierarchy_last).
            if ( hierarchy_last )
            {
                std::string out_top = config.output_prefix + ".species_top10_strains.tsv";
                
                std::ofstream ot( out_top );

                ot << "species_id\tstrain_id\tunique_support\tmulti_support\n";

                for ( auto const& [sp, m] : merged_support )
                {
                    // sort strains: unique first desc, then (unique+multi) desc, then strain id
                    std::vector<std::pair<std::string, detail::StrainSupport>> arr;
                    arr.reserve( m.size() );
                    for ( auto const& [st, sup] : m ) arr.push_back({st, sup});

                    std::sort(arr.begin(), arr.end(),
                              [&](auto const& a, auto const& b)
                              {
                                  auto const& A = a.second;
                                  auto const& B = b.second;
                                  if ( A.unique_support != B.unique_support ) return A.unique_support > B.unique_support;
                                  uint32_t As = A.unique_support + A.multi_support;
                                  uint32_t Bs = B.unique_support + B.multi_support;
                                  if ( As != Bs ) return As > Bs;
                                  return a.first < b.first;
                              });

                    size_t K = std::min<size_t>( config.species_topk, arr.size() );
                    for ( size_t i=0; i<K; ++i )
                    {
                        ot << sp << '\t' << arr[i].first << '\t'
                           << arr[i].second.unique_support << '\t'
                           << arr[i].second.multi_support << '\n';
                    }
                }
                ot.close();
            }
        ...
        if ( !config.output_prefix.empty() )
        {
            ...
            if ( config.output_all )
                out_all.close();
            if ( species_all_opened )
                out_species_all.close();
        }
        if ( hierarchy_first )
        {
