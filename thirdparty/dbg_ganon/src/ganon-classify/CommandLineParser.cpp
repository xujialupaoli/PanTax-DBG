...
std::optional< Config > CommandLineParser::parse( int argc, char** argv )
{
    ...
    options.add_options()
        ...
        ( "output-species-all", "Output folded species matches file (<prefix>.species.all)", cxxopts::value< bool >() )
        ( "output-species-topk", "Output Top-K strains per species (<prefix>.species_top10_strains.tsv)", cxxopts::value< bool >() )
        ( "species-topk", "Top-K strains per species (default: 10)", cxxopts::value< uint16_t >() )
        ( "multi-pick", "For multi-match reads, pick top-N strains to count multi_support (default: 3)", cxxopts::value< uint16_t >() )
    ...
    if ( args.count( "output-species-all" ) )
        config.output_species_all = args["output-species-all"].as< bool >();
    if ( args.count( "output-species-topk" ) )
        config.output_species_topk = args["output-species-topk"].as< bool >();
    if ( args.count( "species-topk" ) )
        config.species_topk = args["species-topk"].as< uint16_t >();
    if ( args.count( "multi-pick" ) )
        config.multi_pick = args["multi-pick"].as< uint16_t >();
