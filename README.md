...
- [Installation](#installation)
  ...
  - [Install the current GitHub revision](#install-the-current-github-revision)
- [Commands](#commands)
...
### Install the current GitHub revision

Use this route when you want the latest repository code rather than only the released Conda package. First complete the Conda installation above and keep the environment activated. Then run:
...
This keeps the runtime components supplied by the Conda environment and updates the PanTax-DBG Python interface and workflow code from the repository checkout.
...
```text
usage: pantax-dbg [-h] <command> ...

PanTax-DBG: a robust and accurate species-level metagenomic profiler.

positional arguments:
  <command>
    build-custom  Build custom PanTax-DBG databases.
...
| --- | --- | --- |
| `-k`, `--kmer-size` | `31` | k-mer size used to split reference sequences during database construction. |
| `-w`, `--window-size` | `51` | Window size used for minimizer-based database construction. |
| `--input-file` | `example/exampleDB_input_genomes.txt` | Headerless genome manifest with genome path, strain name and strain taxid. |
| `--taxonomy-files` | `nodes.dmp names.dmp` | Taxonomy structure and name files used to define the database hierarchy. |
| `--db-prefix` | `example/pantaxdbg_DB/pantaxdbg_db` | Output prefix for all database files generated by this command. |
| `--level` | `strain` | Maximum taxonomic level included in the database. |
| `--genome-size-files` | `example/my_genome_sizes.ncbi.tsv.gz` | Species genome-size table used for abundance normalization. |
| `-t`, `--threads` | `16` | Number of worker threads. |
| `--verbose` | enabled | Print more detailed progress information during database construction. |
...
| Option | Example value | Meaning |
| --- | --- | --- |
| `-r`, `--reads` | `example/read1.fq`, `example/read2.fq` | Input paired-end reads. Provide the two mates consecutively in R1/R2 order. |
| `--db-prefix` | `example/pantaxdbg_DB/pantaxdbg_db` | Prefix of the database generated by `pantax-dbg build-custom`. |
| `--ref-info` | `example/example_profile_genome_info.local.txt` | Headered strain-to-species reference table with genome paths used for graph refinement. |
| `--output` | `example/profile_res` | Output directory for abundance profiles and intermediate reports. |
| `--threads` | `16` | Number of worker threads. |
| `-k`, `--kmer-size` | `31` | k-mer size used in the ccDBG-based profiling step. |
| `--strain-min-reads` | `5.0` | Minimum assigned-read support required before strain-group ranking. |
| `--strain-topk` | `5` | Maximum number of retained strain groups per species after ranking. |
| `--strain-div` | `5.0` | Divisor used in the secondary strain-support filtering step. |
| `--r1-large-n` | `100` | Candidate-set size above which large-set candidate filtering is applied. |
| `--r1-topk-large` | `10` | Number of strain candidates retained per species in large-set mode. |
| `--r1-min-abundance` | `1e-7` | Minimum first-stage species relative abundance retained for graph refinement. |
